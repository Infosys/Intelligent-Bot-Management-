...
#================================================================================================
#
# FILE: healthCheckDeployedUiPathDevicesStatus.ps1
#
# USAGE: PowerShell.exe -File <location of the script>\healthCheckDeployedUiPathDevicesStatus.ps1 -authenticationURI <Authentication URI> -deviceSerivceURI <Device Service URI>  -clientId <Client Id> -refreshToken <Refresh Token> -uiPathTenantName <uiPath TenantName> -userName <User Name> -password <password>
#
# DESCRIPTION: This script generates an authentication token using the authentication URL
# and passes this token to get devices details with status from Orchestrator
#
# PRE-REQUISITES: 
#     Machine must be Windows machine
#
# INPUT: authenticationURI,deviceSerivceURI,clientId,refreshToken,uiPathTenantName,userName,password 
# NOTE: Kindly enter the inputs in following sequence :
# authenticationURI,deviceSerivceURI,clientId,refreshToken,uiPathTenantName,userName,password 
# Example: authenticationURI = "https://account.uipath.com/oauth/token"
#          Final deviceSerivceURI = #https://cloud.uipath.com/infoszwekhof/InfosysDefapyud470494/odata/Sessions 
#https://cloud.uipath.com/infoszwekhof/InfosysDefapyud470494/odata/Sessions?$select=State&$expand=Robot
# OUTPUT: List of devices attached to the Orchestrator.
...
elseif (-not $PSBoundParameters.ContainsKey('deviceSerivceURI')){
	$date = timestamp
	$errordata = "[ERROR001]: [$($date)]: [Invalid Arguments]: Please pass required parameter - deviceSerivceURI"
    Write-Output ("error=" + $errordata)
	break
...
# DESCRIPTION: Function will get device details from the Orchestrator
# INPUT: authorizationToken
# RETURN: NA.

Function getDevicesDetails($authorizationToken)
{
Try{
	...
	if ($httpRequest.statusCode -eq 200) {		
		...
		write-output("[ERROR005]: [$($date)]:[Service Invocation Error]: Error occurred in getDevicesDetails method.")
	}
	
	write-output("DevicesStatus:" + $httpRequest)
...
Catch
{
		...
		$errordata = "[ERROR006]: [$($date)]: [Service Invocation Error]: Exception occurred in getDevicesDetails method. Exception is : "  + $_.Exception.Message
		write-output($errordata)
}
finally
{
