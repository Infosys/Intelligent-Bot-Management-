...
#================================================================================================
#
# FILE: getUiPathRobotsDetails.ps1
#
# USAGE: PowerShell.exe -File <location of the script>\getUiPathRobotsDetails.ps1 -authenticationURI <Authentication URI> -robotSerivceURI <Robots Service URI>  -clientId <Client Id> -refreshToken <Refresh Token> -uiPathTenantName <uiPath TenantName> -userName <User Name> -password <password>
#
# DESCRIPTION: This script generates an authentication token using the authentication URL
# and passes this token to get Robot details from Orchestrator
#
# PRE-REQUISITES: 
#     Machine must be Windows machine
#
# INPUT: authenticationURI,robotSerivceURI,clientId,refreshToken,uiPathTenantName,userName,password
# NOTE: Kindly enter the inputs in following sequence :
# authenticationURI,robotSerivceURI,clientId,refreshToken,uiPathTenantName,userName,password 
# Example: authenticationURI = "https://account.uipath.com/oauth/token" or https://vtlbpmct2-01.ad.infosys.com/api/account/authenticate
#          Final robotSerivceURI = "https://cloud.uipath.com/infoszwekhof/InfosysDefapyud470494/odata/Robots"
# OUTPUT: List of Robots attached to the Orchestrator.
...
elseif (-not $PSBoundParameters.ContainsKey('robotSerivceURI')){
	$date = timestamp
	$errordata = "[ERROR001]: [$($date)]: [Invalid Arguments]: Please pass required parameter - robotSerivceURI"
    Write-Output ("error=" + $errordata)
	break
...
# DESCRIPTION: Function will get Robot details from the Orchestrator
# INPUT: authorizationToken
# RETURN: NA.

Function getRobotDetails($authorizationToken)
{
Try{
	...
	if ($httpRequest.statusCode -eq 200) {		
		...
		write-output("[ERROR005]: [$($date)]:[Service Invocation Error]: Error occurred in getRobotDetails method.")
	}
	
	write-output("RobotsDetails:" + $httpRequest)
...
Catch
{
		...
		$errordata = "[ERROR006]: [$($date)]: [Service Invocation Error]: Exception occurred in getRobotDetails method. Exception is : "  + $_.Exception.Message
		write-output($errordata)
}
finally
{
