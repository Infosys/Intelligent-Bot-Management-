...
#
# USAGE: PowerShell.exe -File <location of the script>.\fetchEnvironmentDetails.ps1 -MachineName
#
...
If (-not $PSBoundParameters.ContainsKey('machineName')){
	...
	$errordata = "[ERROR001] Please pass all the required parameters"
    Write-Output ("error=" + $errordata)
	
}
Try{
$value1 = Invoke-command -ComputerName $machineName -ErrorAction Stop -scriptblock { Get-ItemProperty -Path "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*"  | Select-Object DisplayName,DisplayVersion,Publisher,InstallDate }
$value2 = Invoke-command -ComputerName $machineName -ErrorAction Stop -scriptblock { Get-ItemProperty -Path "HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*"  | Select-Object DisplayName,DisplayVersion,Publisher,InstallDate }
$value3 = Invoke-command -ComputerName $machineName -ErrorAction Stop -scriptblock  { Get-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*"  | Select-Object DisplayName,DisplayVersion,Publisher,InstallDate }
}
...
Try{
$OsDetails = Invoke-command -ComputerName $machineName -ErrorAction Stop -scriptblock { Get-WmiObject Win32_OperatingSystem  | Select PSComputerName, Caption, OSArchitecture, Version, BuildNumber }
...
Try{
$ScreenResolution = Invoke-command -ComputerName $machineName -ErrorAction Stop -scriptblock { Get-WmiObject Win32_DisplayConfiguration  | Select PelsHeight, PelsWidth }
$trialJSON += [pscustomobject]@{
