...
namespace Infosys.Lif.LegacyWorkbench
{
    ...
    /// </summary>
    public class Cobol:IModelObjectRetriever
    {
        ...
        private Entity Translate(string pathOfCobolCopyBook)
        {
            lengthCalculator = 0;
            ...
            //cdsrajan
            string[] splitLinkageSection;
            try
            {
                splitLinkageSection = linkageSection.Split(splitters, StringSplitOptions.RemoveEmptyEntries);
            }
            catch
            {
            ...
            //get all data items below Linkage Section
                ...
                if (Int32.TryParse(currDeclarationTokens[0], out currLevelOfDeclaration))
                {
                    if (prevItem.DeclarationLevel < currLevelOfDeclaration)
                    {
                        ...
                        if (currDeclarationTokens.Length > 2 && currDeclarationTokens[2].ToLowerInvariant().Equals("pic"))
                        {
                            ...
                            dataItem = BuildDataItem(currDeclarationTokens, dataItem);
                        }
                        else
                        {
                            dataItem = new DataItem();
                        ...
                        prevItem.declarationItem.ItemType = DataItem.DataItemType.GroupItem;
            ...
            //remove Linkage Section
            Entity tempEnt = new Entity();
            foreach (DataItem d in currEntity.DataItems[0].GroupItems)
            {
                tempEnt.DataItems.Add((DataItem)d);
            }
            currEntity.DataItems.Clear();

            for (int count = 0; count < tempEnt.DataItems.Count; count++)
            {
                currEntity.DataItems.Add(tempEnt.DataItems[count]);
            }
            return currEntity;
        ...
        //to calculate position of each data item accd to length of previous data item.
        int lengthCalculator = 0;

        private DataItem BuildDataItem(string[] currDeclarationTokens, DataItem dataItem)
        {
            ...
            if (currDeclarationTokens[2].ToLowerInvariant() == "pic")
            {
                ...
                if (splitToken[0].StartsWith("x",
                    StringComparison.InvariantCultureIgnoreCase))
                {
                    dataItem.ItemType = DataItem.DataItemType.String;
                }
                else
                {
                    //if (splitToken[0].StartsWith("-",
                    ...
                    //else
                    {
                        dataItem.ItemType = DataItem.DataItemType.Integer;
                    }
                }
                //If only x is mentioned
                if (splitToken.Length > 1)
                ...
                lengthCalculator += (dataItem.Length * dataItem.NumberOfOccurences);
            }
        ...
        protected static string ParseToCorrectCase(string variableName)
        {
            ...
            // split cahracter. and append these tokens using the _ as seperator.
            char[] splitChars = { '-' };
        ...
        public Entity RetrieveModelObjectDetails(string filePath)
        {
